...
export const TokenView = () => {
  const tmt_tokens = localStorage.getItem('tmt_tokens') || JSON.stringify([]);
  const logs = JSON.parse(tmt_tokens);
      ...
      {logs?.reverse()?.map((log, index) => {
        return <Accordion key={index} expanded={expanded === 'panel'+index} onChange={handleChange('panel'+index)}>
          ...
          >
            ...
            <Typography sx={{ color: 'text.secondary' }}> Quantity: {log?.nft?.nftIds?.length}</Typography>
          </AccordionSummary>
          <AccordionDetails>
